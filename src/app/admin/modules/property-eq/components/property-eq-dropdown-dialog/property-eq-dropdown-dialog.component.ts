...
@Component({
  selector: 'app-property-eq-dropdown-dialog',
  templateUrl: './property-eq-dropdown-dialog.component.html',
  styleUrls: ['./property-eq-dropdown-dialog.component.scss'],
})
export class PropertyEqDropdownDialogComponent implements OnInit {
  ...
  seoData: any;
  constructor(
    public dialogRef: MatDialogRef<PropertyEqDropdownDialogComponent>,
  ...
  getSeoDetailsByPropertyId(propertyId: any) {
    ...
    this.propertyEqService.getPropertyEqSeodata(propertyId).subscribe({
      next: (res: any) => {
  ...
  updateSEO(payload: any) {
    ...
    this.propertyEqService
      .updatePropertyEqSeo(this.formConfig.id, payload)
      .subscribe({
        next: (res: any) => {
          this.loaderService.hide();
