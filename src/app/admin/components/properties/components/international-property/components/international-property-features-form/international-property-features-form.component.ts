...
@Component({
  selector: 'app-international-property-features-form',
  templateUrl: './international-property-features-form.component.html',
  styleUrls: ['./international-property-features-form.component.scss']
})
export class InternationalPropertyFeaturesFormComponent extends InternationalPropertyComponent {
    ...
    this.propertyService.addUpdatePropertyFeatures(payload).subscribe({
      next: (res) => {
       ...
       // this.toasterService.success(res.message);
        this.changeStep(ROUTE.INTERNATIONAL_PROPERTY_CONFIRMATION, res.message);
      },
      error: (error) => {
        this.loaderService.hide();
        error.error.errors
  ...
  breeamCertificationRatingChange(event: any, featureForm: any) {
    if(event?.Id){
    this.breeamCertificationRatingId = event.Id;
      featureForm.get('Value').setValue(event.Id);
    }
    else{
      featureForm.get('Value').setValue(null);
    }
  }
